...
export interface FriendsProvider {
  ...
  /**
   * Дальше — то, чего у демо-провайдера по-настоящему нет. Методы
   * необязательные: экран спрашивает наличие, а не тип провайдера,
   * и одинаково работает с сервером и без него.
   */

  /** Отправить свою позицию. Частоту решает вызывающий, см. publish.ts. */
  publishPosition?(point: LngLat): Promise<void>;
