...
#
#   npm run setup:ios                — сборка для разработки, на свой телефон
#   npm run setup:ios -- --release   — самостоятельная сборка, на чужой телефон
#
...
# Debug-сборка берёт JS с Mac по сети: удобно править код, но на чужом
# телефоне она превращается в тыкву, как только человек ушёл из дома.
# Release зашивает JS внутрь — приложение работает само и где угодно.
CONFIGURATION="Debug"
for arg in "$@"; do
  case "$arg" in
    --release) CONFIGURATION="Release" ;;
    *)
      printf '\033[31mНеизвестный аргумент: %s\033[0m\n' "$arg" >&2
      printf 'Доступен только --release — сборка для чужого телефона.\n' >&2
      exit 1
      ;;
  esac
done
...
# Ключи Supabase вшиваются в бандл при сборке. Без них приложение молча
# переходит на демо-друзей, и человек с чужого телефона будет разглядывать
# выдуманных людей вместо родственников, не понимая почему.
if [ ! -f .env ] || ! grep -q '^EXPO_PUBLIC_SUPABASE_URL=https' .env 2>/dev/null; then
  bold "  Внимание: в .env нет адреса Supabase — живые друзья работать не будут.
  Приложение соберётся, но покажет демо-друзей. Скопируйте .env.example
  в .env и подставьте свои значения, если друзья нужны."
fi
...
# Режим разработчика. Без него xcodebuild компилирует всё до конца и только
# потом отваливается по таймауту («Timed out waiting for all destinations»),
# причём настоящая причина прячется в самом низу вывода. Двадцать минут
# компиляции ради строчки, которую видно заранее.
devices_json="$(mktemp)"
if xcrun devicectl list devices --json-output "$devices_json" >/dev/null 2>&1 &&
   grep -q '"developerModeStatus"[[:space:]]*:[[:space:]]*"disabled"' "$devices_json"; then
  bold "  На подключённом телефоне выключен «Режим разработчика» — сборка
  до него не дойдёт. Включите: Настройки → Конфиденциальность и
  безопасность → в самом низу «Режим разработчика» → перезагрузка →
  после разблокировки подтвердить «Включить».

  Пункт появляется в настройках только после первой попытки установки:
  если его там нет, доведите этот запуск до конца и включите потом."
fi
rm -f "$devices_json"
...
if [ "$CONFIGURATION" = "Release" ]; then
  step "Собираю самостоятельную версию и ставлю на телефон"
  bold "  Дольше обычного: JS зашивается внутрь приложения."
else
...
Готово. Приложение работает само по себе: ни Mac, ни общий Wi-Fi ему
больше не нужны — можно отключать кабель и уходить гулять.

На телефоне остался один шаг: Настройки → Основные → VPN и управление
устройством → ваш Apple ID → «Доверять».

Через 7 дней приложение перестанет открываться — это лимит бесплатной
подписи Apple, а не баг. Лечится повторным подключением телефона кабелем
и командой: npm run setup:ios -- --release
